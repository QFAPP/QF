...
if __name__ == "__main__":
    ...
    parser.add_argument("-sopt", "--source-options", dest="source_options",
                        action="store", nargs="?", const="", default="",
                        help="The options for the source plugin")
    parser.add_argument("-dopt", "--destination-options",
                        dest="destination_options", action="store", nargs="?",
                        const="", default="",
    ...
    if not sourcePluginInfo:
        print(
            "Cannot find the specified source plugin: {0}".format(args.source))
        exit()
    sourcePlugin = sourcePluginInfo.plugin_object
    sourcePlugin.parse_options(args.source_options)

    # Set the destination plugin
    destinationPluginInfo = kbManager.manager.getPluginByName(args.destination,
                                                              "Destination")
    if not destinationPluginInfo:
        print("Cannot find the specified destination plugin: {0}".format(
            args.destination))
        exit()
